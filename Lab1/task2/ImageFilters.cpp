...
int median_filter(const cv::Mat& src, cv::Mat& out, const int& kernel_size) {
    ...
    for(int i = 0; i < src.rows; i += kernel_size) {
        for(int j = 0; j < src.cols; j += kernel_size) {
            ...
            // Since we know that the kernel is a square with odd sides, the num of elements is odd, so median always found
            // If I had even number of elements the median would have been the average of the 2 number in the middle
            unsigned char median = kernel_elements[(numElem_kernel/ 2) + 1];
